...
def getComplaints(complaints_file):
  ...
  with open(complaints_file) as f:
    ...
    for row in reader:
      ...
      if incident_zip in zipBorough and complaint_type == 'Indoor Air Quality':
...
def drawPlot(shapeFilename, zipBorough):
  ...
  # Creates the Plot
  output_file("311_countsZip_airquality_indoor.html")
  ...
  patches(polygons['lng_list'],
          ...
          plot_width=1100,
          plot_height=700,
          title="Number of Indoor Air Quality Complaints per Zip Code in 2014",
  )

  # Draws Points on top of map.
  hold()

  airquality_indoor = scatter(center_lng,
          center_lat,
          fill_color='orange',
          color='orange',
          line_alpha=0.1,
  ...
  show(airquality_indoor)

  script, div = components(airquality_indoor, CDN)

  embed_txt = open('airquality_indoor_embed.txt', 'w')
